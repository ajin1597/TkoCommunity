...
const CommunityPage = () => {
  ...
  const test = "가 나다 라 마 바 사아 자 차 카 타파 하 타파 하 타파 하 타파 하 타파 하 타파 하 타파 하 타파 하 타파 하 타파 하 "
      ...
      <Layout title="커뮤니티">
        <div id="커뮤니티 페이지 전체 레이아웃">
          ...
          >
            ...
            <div id="커뮤니티 게시물 포멧" className="w-[100%] bg-blue-300 flex flex-wrap justify-center">
              {noticData.map((notic, idx) => {
                return (
                  <div key={idx} className="m-6 w-[250px] ">
                    <nav>
                      <ul>
                        <li className="border-black border-2 rounded-2xl">
                          <Link to={`/SingleNoticPage/${notic.num}`}>
                            <div id="작성자" className="flex justify-center">{notic.writer}</div>
                            <div id="게시물번호" className="flex justify-center">{notic.num}</div>
                            <div id="제목글" className="flex justify-center">{notic.title}</div>
                            <div id="작성시간" className="flex justify-center">{notic.date}</div>
                            <div id="게시물내용" className="h-[100px] px-4 py-2 text-left overflow-ellipsis overflow-hidden flex flex-wrap justify-center">{test}...</div>
                          </Link>
                        </li>
                      </ul>
                    </nav>
